...
def login_gen():
    ...
    while True:
        l = "".join(random.choices(string.ascii_letters.lower() + string.digits, k=random.randint(3, 16)))
        yield l
...
def custom_string_gen(big_letters: bool, digits: bool, gen_range: List[int]):
    ...
    while True:
        l = "".join(random.choices(((string.ascii_letters.lower() if not big_letters else string.ascii_letters) + string.digits if digits else string.ascii_letters), k=random.randint(gen_range[0], gen_range[1])))
        yield l
...
def domain_gen():
    ...
    while True:
        d = "".join(random.choices(string.ascii_letters.lower() + string.digits, k=random.randint(2,4)))
        yield d
...
def email_gen():
    ...
    while True:
        email = "".join(login_gen().__next__() + "@"+ domain_gen().__next__() + country_gen().__next__())
        yield email
